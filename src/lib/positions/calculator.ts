 ...
 */
function calculateArchetypeScore(rawStats: PlayerStats, weights: Partial<Record<StatsKey, number>>): number {
...
export function getTopArchetypes(
    ...
    return allArchetypes.slice(0, maxCount).filter((item) => item.score >= threshold);
}
 ...
 */
export function getAllPositionArchetypeGroups(archetypeScores: PlayerArchetypeScores): Array<{
    position: Position;
    bestScore: number;
