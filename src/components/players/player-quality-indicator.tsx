    ...
    return (
        <div className={`flex flex-col ${config.gap}`}>
            <div className={`flex items-center ${config.gap}`}>
                {showLabel && (
                    <span className={`font-semibold ${config.text} ${quality.solidBg}`}>{quality.label}</span>
                )}
            </div>
            {showDescription && <span className={`text-xs text-muted-foreground`}>{quality.label}</span>}
        </div>
    );
};
