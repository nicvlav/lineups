...
import { User, Trash2, TrendingUp } from "lucide-react";
import { Position } from "@/types/positions";
import { calculateScoresForStats } from "@/lib/utils/player-scoring";
import {
  calculateAllRelativeScores,
  applyVisualScaling
} from "@/lib/utils/relative-scoring";
...
const OverviewTabComponent: React.FC<OverviewTabComponentProps> = ({
  player,
  fullPlayer,
}) => {
      ...
      <div className="bg-card border rounded-lg p-4">
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-center">
            <span className="text-sm text-muted-foreground">Current Position ({POSITION_NAMES[player.exactPosition]}) </span>
            <span className="text-sm font-bold">{currentPositionFit.toFixed(0)}%</span>
          </div>
          <div className="w-full bg-muted rounded-full h-2">
            <div
              className={`h-2 rounded-full transition-al ${getArchetypeBarColor(
                currentPositionFit,
              )}`}
...
const SwapTabComponent: React.FC<SwapTabComponentProps> = ({
  player,
  players,
  onClose,
}) => {
          ...
          {availablePlayers.map((p) => {
            ...
            return (
              <button
                ...
                className={`w-full p-3 text-left rounded-lg border transition-colors ${isInGame
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  {isInGame && (
                    <span className="text-xs text-red-500">In game</span>
                  )}
                </div>
              </button>
            );
...
const PitchPlayerDialog: React.FC<PlayerDialogProps> = ({
  player,
  isOpen,
  onClose,
}) => {
      ...
      <div className="flex flex-col h-[85vh] max-w-lg mx-auto">
        ...
        <div className="flex items-center gap-4 p-4 border-b bg-linear-to-r from-background to-muted/20">
          <div className="relative">
            <div className={`w-16 h-16 rounded-full flex items-center justify-center text-white ${player.team === 'A' ? 'bg-cyan-500' : 'bg-lime-500'
              }`}>
              {fullPlayer?.avatar_url ? (
                <img src={fullPlayer.avatar_url} alt={playerName} className="w-full h-full rounded-full object-cover" />
              ) : (
                <User size={32} />
              )}
            </div>
