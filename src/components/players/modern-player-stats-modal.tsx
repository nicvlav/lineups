...
import ModernPanel, { PanelSection } from '@/components/shared/modern-panel';
...
import { getPrimaryArchetypeId, getPlayStyleBuzzwords, classifyPlayerByZone } from '@/lib/player-quality';
...
const ModernPlayerStatsModal: React.FC<ModernPlayerStatsModalProps> = ({
  ...
  const buzzwords = primaryArchetypeId
    ? getPlayStyleBuzzwords(primaryArchetypeId)
    : [];

  let zoneScores = structuredClone(emptyZoneScores);
  ...
  // Get top position groups with threshold
  const bestScore = topPositionGroups.length ? topPositionGroups[0].archetypes.reduce((prev, current) => {
    ...
    <Modal title={player.name} isOpen={isOpen} onClose={onClose}>
      <div className="flex flex-col h-[85vh] min-w-[300px] max-w-[1100px] ">
        ...
        <div className="mt-4 space-y-4 pb-4 pr-4 pl-4 overflow-y-auto">
          ...
          <PanelSection
            title="Position Fit"
            subtitle={showAllPositions ? "All positions with archetype scores" : "Top positions shown relative to best archetype"}
            headerAction={
              <button
          ...
          >
            <div className="space-y-3 ">
              {(showAllPositions ? allPositionGroups : topPositionGroups).map((posGroup) => {
                ...
                const bestArchetypeId = 'bestArchetypeId' in posGroup
                  ? posGroup.bestArchetypeId
                  : posGroup.archetypes.find(a => a.isBest)?.archetypeId ?? '';
                const archetypes = 'allArchetypes' in posGroup
                  ? posGroup.allArchetypes
                  : posGroup.archetypes;

                const archetypeData = getArchetypeById(bestArchetypeId);
                const bestArchetype = archetypes.find(a => a.archetypeId === bestArchetypeId)
                  ?? archetypes[0];

                return (
                  <ModernPanel key={position} variant="card" padding="md">
                    {/* Position Header */}
          ...
          {isAdmin(user?.id) && showDetailedStats && (
            <PanelSection title="Detailed Statistics" subtitle="Raw player attributes">
              <ModernPanel variant="glass" padding="md">
                {/* Summary Stats */}
